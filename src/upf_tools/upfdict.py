...
class UPFDict(OrderedDict):
    ...
    def __eq__(self, other):
        """Compare two UPFDict objects for equality."""
        if not isinstance(other, UPFDict):
            return False
        if len(self) != len(other):
            return False

        def compare_values(val1, val2):
            """Compare two values for equality, accounting for different possible types of values."""
            if isinstance(val1, np.ndarray) and isinstance(val2, np.ndarray):
                return np.array_equal(val1, val2)
            elif isinstance(val1, (list, tuple, OrderedDict)):
                return _compare_iterables(val1, val2)
            else:
                return val1 == val2
        ...
        for key, value in self.items():
            if key not in other:
                return False
            if not compare_values(value, other[key]):
                return False
        return True


def _compare_iterables(val1, val2):
    """Compare iterables (lists, tuples, or nested OrderedDicts)."""
    if len(val1) != len(val2):
        return False
    for v1, v2 in zip(val1, val2):
        if isinstance(v1, (list, tuple, OrderedDict)):
            if not _compare_iterables(v1, v2):
                return False
